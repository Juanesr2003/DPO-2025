...
public class Aerolinea
{
    ...
    public Vuelo getVuelo(  String codigoRuta, String fechaVuelo )
    {
        ...
        {
        	if(vuelo.getFecha().equalsIgnoreCase(fechaVuelo)) {

	            if (vuelo.getRuta().getCodigoRuta().equalsIgnoreCase(codigoRuta))
	            {
	                return vuelo;
            	}
    ...
    {
        Collection<Tiquete> tiquetesAdevolver = new ArrayList<>();
        for (Vuelo vuelo : vuelos)
        {
            Collection<Tiquete> tiquetes = vuelo.getTiquetes();
            tiquetesAdevolver.addAll(tiquetes);
        }
        return tiquetesAdevolver ;
    ...
    {
        IPersistenciaAerolinea cagador = CentralPersistencia.getPersistenciaAerolinea(tipoArchivo);
        cagador.salvarAerolinea(archivo, this);
    }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
	    ...
	    Avion avion = obtenerAvionPorNombre(nombreAvion);
	
	    if (estaAvionDisponible(avion, fecha, ruta)) {
	        Vuelo nuevoVuelo = new Vuelo(ruta, fecha, avion);
	        vuelos.add(nuevoVuelo);
	    } else {
	        throw new Exception("Este avión está ocupado para otro vuelo en el mismo intervalo de tiempo :c");
	    }
	}
	
	// Busca un avión por su nombre en la lista de aviones registrados
	private Avion obtenerAvionPorNombre(String nombreAvion) {
	    for (Avion avionExistente : aviones) {
	        if (avionExistente.getNombre().equals(nombreAvion)) {
	            return avionExistente;
	        }
	    }
	    return new Avion(nombreAvion, 0); 
	}
	
	// Verifica si el avión está disponible en la fecha y horarios especificados
	private boolean estaAvionDisponible(Avion avion, String fecha, Ruta nuevaRuta) {
	    for (Vuelo vueloExistente : vuelos) {
	        if (vueloExistente.getAvion().equals(avion) && vueloExistente.getFecha().equals(fecha)) {
	            int horaSalidaExistente = Integer.parseInt(vueloExistente.getRuta().getHoraSalida());
	            int horaLlegadaExistente = Integer.parseInt(vueloExistente.getRuta().getHoraLlegada());
	
	            int nuevaHoraSalida = Integer.parseInt(nuevaRuta.getHoraSalida());
	            int nuevaHoraLlegada = Integer.parseInt(nuevaRuta.getHoraLlegada());
	
	            if (!(horaLlegadaExistente <= nuevaHoraSalida || horaSalidaExistente >= nuevaHoraLlegada)) {
	                return false; 
	            }
	        }
	    }
    ...
    {  
        ...
        int mesVuelo = obtenerMesDesdeFecha(fecha);
        CalculadoraTarifas calculadora;
        if ( mesVuelo == 6 || mesVuelo == 7 || mesVuelo == 8 ||mesVuelo == 12)
        {   
            calculadora = new CalculadoraTarifasTemporadaAlta(vuelo, cliente); 
        }else 
        {
            calculadora = new CalculadoraTarifasTemporadaBaja(vuelo, cliente);
        }
    ...
    /** Extrae el mes de la fecha en formato "YYYY-MM-DD"
     * 
     * @param fecha
     * @return int
     */
    private int obtenerMesDesdeFecha(String fecha) {
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
        ...
        if (vuelo == null) {
            throw new IllegalArgumentException("No se encontró el vuelo con la ruta y fecha especificadas.");
        }
    ...
    { Cliente cliente = getCliente(identificadorCliente);
	    ...
	    if (cliente == null) {
	        throw new IllegalArgumentException("El cliente con el identificador proporcionado no existe.");
	    }
